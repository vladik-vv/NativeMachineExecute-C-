...
struct Change{ 
        ...
        switch (mode){
            ...
            case _rand:{
                ...
                switch (typeArg1){
                    ...
                    default: num1 = Convert.ToInt32(nameArg1); break;
                }
            ...
            case _rss:{
                ...
                if (byteArrs.ContainsKey(nameArg1)){
                    byteArrs[nameArg1] = byteArrs[nameArg1].OrderBy(x => Guid.NewGuid()).ToArray();
                    return;
                } else if (shortArrs.ContainsKey(nameArg1)){
                    shortArrs[nameArg1] = shortArrs[nameArg1].OrderBy(x => Guid.NewGuid()).ToArray();
                    return;
                } else if (floatArrs.ContainsKey(nameArg1)){
                    floatArrs[nameArg1] = floatArrs[nameArg1].OrderBy(x => Guid.NewGuid()).ToArray();
                    return;
                } else if (doubleArrs.ContainsKey(nameArg1)){
                    doubleArrs[nameArg1] = doubleArrs[nameArg1].OrderBy(x => Guid.NewGuid()).ToArray();
                    return;
                } else if (stringArrs.ContainsKey(nameArg1)){
                    stringArrs[nameArg1] = stringArrs[nameArg1].OrderBy(x => Guid.NewGuid()).ToArray();
                    return;
                } else if (Matrix2_q.ContainsKey(nameArg1)){
                    ShuffleMatrix(Matrix2_q[nameArg1], _rnd);
                    return;
                } else if (Matrix2_s.ContainsKey(nameArg1)){
                    ShuffleMatrix(Matrix2_s[nameArg1], _rnd);
                    return;
                }
                return;
            } 
    ...
    private static void ShuffleArray<T>(T[] array, Random rnd){
        int n = array.Length;
        while (n > 1){
            n--;
            int k = rnd.Next(n + 1);
    ...
    private static void ShuffleMatrix<T>(T[,] matrix, Random rnd){
